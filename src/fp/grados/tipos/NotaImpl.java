...
public class NotaImpl implements Nota{
	...
	public NotaImpl(String s){
		...
		if(trozos.length!=5){
			throw new IllegalArgumentException("NotaImpl.NotaImpl: Cadena no válida");
		}
	...
	private void checkMencionHonor(Double val, Boolean men){
		if(val<9 && men){
			throw new ExcepcionNotaNoValida("NotaImpl.checkMencionHonor: La nota no alcanza para ser concedida una mención de honor");
		}
	}
